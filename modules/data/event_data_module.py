"""
事件数据 DataModule 封装（LightningDataModule）。

- 事件相机数据（虚拟 / DSEC），统一对外提供 train/val/test dataloader；
- 支持两种采样模式：
    1) STREAM  流式采样（基于 IterableDataset）；
    2) RANDOM  随机访问采样（基于 RandomAccessDataset）；
- 支持 MIXED 训练模式：在一个 epoch 中按比例混合 STREAM + RANDOM 两种 loader；
- 负责根据 `config['dataset']` 中的设置，构造真正的底层 Dataset / DataPipe 并包一层 DataLoader。
...
def get_dataloader_kwargs(
    ...
    """
    根据采样模式 / 数据集模式，返回构造 DataLoader 所需的关键参数字典。

    这样 `train_dataloader / val_dataloader / test_dataloader` 里就不需要充满 if-else，
    可读性会更好一些。
    """
...
class DataModule(pl.LightningDataModule):
    """
    事件深度估计项目的数据模块（LightningDataModule）：统一管理

    - 训练集：支持 STREAM / RANDOM / MIXED 三种采样策略；
    - 验证 / 测试集：支持 STREAM / RANDOM 两种采样策略；
    - train/val/test 各自返回合适的 DataLoader（或 mapping[SamplingMode -> DataLoader]）。

    参数
    ------
    dataset_config : Dict[str, Any]
        来自 YAML 的 `config['dataset']` 字典，内部字段决定采样方式等；
    num_workers_train, num_workers_eval : int
        训练 / 验证阶段的 DataLoader num_workers；
    batch_size_train, batch_size_eval : int
        训练 / 验证阶段的 batch size。
    """
    ...
    def __init__(
        ...
        self.sampling_mode_2_dataset: Dict[DatasetSamplingMode, Any] = dict()
        self.sampling_mode_2_train_workers: Dict[DatasetSamplingMode, int] = dict()
        self.sampling_mode_2_train_batch_size: Dict[DatasetSamplingMode, int] = dict()
        self.validation_dataset = None
        self.test_dataset = None

    def get_dataloading_hw(self):
        """返回 dataloader 中使用的 (H, W)，会考虑是否下采样 by factor 2。"""
    ...
    def set_mixed_sampling_mode_variables_for_train(self):
        ...
        # 允许 num_workers=0/1（如本地测试），仅当 >=2 时再按比例分配
        ...
        # Set num workers: 0/1 时全部给 stream，避免出现负数 worker
        if self.overall_num_workers_train < 2:
            workers_rnd = 0
            workers_str = self.overall_num_workers_train
        else:
            workers_rnd = min(
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        if stage == "fit":
            ...
            if self.train_sampling_mode in (DatasetSamplingMode.RANDOM, DatasetSamplingMode.MIXED):
                self.sampling_mode_2_dataset[DatasetSamplingMode.RANDOM] = build_random_access_dataset(
                    dataset_mode=DatasetMode.TRAIN,
                    dataset_config=self.dataset_config,
                )
            if self.train_sampling_mode in (DatasetSamplingMode.STREAM, DatasetSamplingMode.MIXED):
                self.sampling_mode_2_dataset[DatasetSamplingMode.STREAM] = build_streaming_dataset(
                    dataset_mode=DatasetMode.TRAIN,
                    dataset_config=self.dataset_config,
    ...
    def train_dataloader(self):
        train_loaders: Dict[DatasetSamplingMode, DataLoader] = dict()
