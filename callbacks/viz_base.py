...
class VizCallbackBase(Callback):
    def __init__(self, config: Dict[str, Any], buffer_entries: Type[Enum]):
        super().__init__()
    ...
    def on_train_batch_end(
        ...
        if not log_train_hd.get('enable', False):
            return

        step = trainer.global_step
        assert log_train_hd.get('every_n_steps', 0) > 0
        if step % log_train_hd.get('every_n_steps', 0) != 0:
            return

        n_samples = log_train_hd.get('n_samples', 4)
    ...
    def on_validation_batch_end(
        ...
        log_freq_val_epochs = log_val_hd.get('every_n_epochs', 1)
        if not log_val_hd.get('enable', False):
            return
        if dataloader_idx > 0:
            raise NotImplementedError
    ...
    def on_validation_epoch_end(self, trainer: pl.Trainer, pl_module: pl.LightningModule) -> None:
        log_val_hd = self.log_config['validation']['high_dim']
        log_n_samples = log_val_hd.get('n_samples', 4)
        log_freq_val_epochs = log_val_hd.get('every_n_epochs', 1)
        ...
        assert isinstance(logger, WandbLogger)
        global_step = trainer.global_step
        self.on_validation_epoch_end_custom(logger, global_step, trainer.current_epoch)
    ...
    @staticmethod
    def ev_repr_to_img(x: np.ndarray, use_last_k_bins: int = 2, denoise_percentile: float = -1):
        """将 stacked histogram 转为 RGB，红蓝配色：红=正极性，蓝=负极性（仅可视化）"""
        ch, ht, wd = x.shape[-3:]
        assert ch > 1 and ch % 2 == 0
        ev_repr_reshaped = rearrange(x, '(posneg C) H W -> posneg C H W', posneg=2)
        num_bins = ev_repr_reshaped.shape[1]
        sel = slice(max(0, num_bins - use_last_k_bins), num_bins)
        img_neg = np.asarray(reduce(ev_repr_reshaped[0, sel], 'C H W -> H W', 'sum'), dtype=np.float32)
        img_pos = np.asarray(reduce(ev_repr_reshaped[1, sel], 'C H W -> H W', 'sum'), dtype=np.float32)
        ...
        if denoise_percentile is not None and 0 < denoise_percentile < 100:
            thr_r = float(np.percentile(r, denoise_percentile))
            thr_b = float(np.percentile(b, denoise_percentile))
            if thr_r > 0:
                r = np.where(r >= thr_r, r, 0.0).astype(np.float32)
            if thr_b > 0:
                b = np.where(b >= thr_b, b, 0.0).astype(np.float32)

        overlay = np.zeros((ht, wd, 3), dtype=np.float32)
        overlay[..., 0] = r
        overlay[..., 2] = b
