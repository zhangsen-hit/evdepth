...
class DataType(Enum):
    ...
    TOKEN_MASK = auto()
    DEPTH = auto()  # Depth ground truth (in log space)
    DEPTH_MASK = auto()  # Valid depth mask


class DatasetType(Enum):
    VIRTUAL = auto()  # Synthetic/Virtual dataset for testing
    DSEC = auto()     # DSEC dataset for depth estimation
    LIOSAM = auto()   # LiOSAM odometry2 depth_dataset (index.txt + npz per frame)
...
LstmState = Optional[Tuple[th.Tensor, ...]]  # (h, c) for ConvLSTM or (h, c, m) for ST-LSTM
