...
class SequenceForRandomAccess(SequenceBase):
    def __init__(self,
        ...
        num_frames = len(self.objframe_idx_2_repr_idx)
        if self.start_idx_offset is None:
            # This leads to actual length of 0:
            self.start_idx_offset = num_frames

        self.length = num_frames - self.start_idx_offset
        
        # For detection tasks, verify label consistency
        if self.label_factory is not None:
