...
class SequenceBase(Dataset):
    ...
    │     └── timestamps_us.npy
    ├── depth_v2 (optional, for depth estimation)
    │   ├── depth_maps.h5
    │   ├── depth_masks.h5
    │   └── timestamps_us.npy
    └── labels_v2 (optional, for detection)
        ├── labels.npz
        └── timestamps_us.npy
    """
    ...
    def __init__(self,
        ...
        assert path.is_dir()
        assert dataset_type in {DatasetType.VIRTUAL, DatasetType.DSEC}, f'{dataset_type} not implemented'
        ...
        # Load object detection labels only if available (not needed for depth estimation)
        if labels_dir.is_dir() and (labels_dir / 'labels.npz').exists():
            ...
            # For depth estimation tasks, we don't need object detection labels
            print(f"  No object detection labels found in {path.name} (depth estimation mode)")
        ...
        # Initialize depth loader if needed
        if self.load_depth:
            with Timer(timer_name='initialize depth loader'):
                ...
                if depth_dir.exists():
                    ...
                    print(f"Depth loader initialized for {path.name}")
                else:
                    print(f"Warning: load_depth=True but depth_v2 directory not found in {path}")
                    print("  Depth data will not be loaded for this sequence.")
